...
// Set Calibrated before calling this function, will need to implement a second
// parameter.
...
void UpdateSampleData(ADCSampleData* my_asd) {
  ...
  my_asd->sum -=
      my_asd->samples[my_asd->index];  // subtract oldest value from sum
  my_asd->samples[my_asd->index] =
      my_asd->current;  // overwrite oldest value with newest
...
void UpdateADCString(ADC_SAMPLE_TYPE my_data, char* my_string,
  ...
  hundred = (temp % 1000) / 100;  //[0-9] in [0-255]
  ten = (temp % 100) / 10;        //[0-9] in [0-255]
  one = (temp % 10);              //[0-9] in [0-255]
  my_string[my_offset] = thousand + ASCII_OFFSET;
  my_string[my_offset + 1] = hundred + ASCII_OFFSET;
